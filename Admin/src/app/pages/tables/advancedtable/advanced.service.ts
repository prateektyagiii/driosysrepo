...
function matches(tables: Table, term: string, pipe: PipeTransform) {
    return tables.Name.toLowerCase().includes(term)
        || tables.Category.toLowerCase().includes(term)
        || tables.Description.toLowerCase().includes(term)
      //  || pipe.transform(tables.InterestId).includes(term);
      //  || tables.CreatedDate.toLowerCase().includes(term)
      //  || tables.IsActive.toLowerCase().includes(term);
}
